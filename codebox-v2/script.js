...
function loadFromStorage() {
  ...
  projectName.textContent =
    localStorage.getItem(STORAGE_KEYS.PROJECT_NAME) || "Untitled Project";
}
...
function setupEventListeners() {
  ...
  htmlEditor.addEventListener("scroll", () =>
    syncScroll(htmlEditor, htmlLineNumbers),
  );
  cssEditor.addEventListener("scroll", () =>
    syncScroll(cssEditor, cssLineNumbers),
  );
  jsEditor.addEventListener("scroll", () =>
    syncScroll(jsEditor, jsLineNumbers),
  );
...
function switchTab(lang) {
  ...
  editorWrappers.forEach((wrapper, index) => {
    const expectedLang = ["html", "css", "js"][index];
    wrapper.classList.toggle("active", expectedLang === lang);
  });
}
...
function updatePreview() {
  ...
  // Don't update if HTML has unclosed tags
  const openTags = (html.match(/<[^/][^>]*>/g) || []).length;
  const closeTags = (html.match(/<\/[^>]+>/g) || []).length;
  const hasOpenScript = html.includes("<script") && !html.includes("</script>");
  const hasOpenStyle = html.includes("<style") && !html.includes("</style>");

  // Skip update if tags are incomplete to prevent rendering artifacts
  if (hasOpenScript || hasOpenStyle || openTags > closeTags + 3) {
    return;
  }
...
function setupResizeHandle() {
  ...
  document.addEventListener("mousemove", (e) => {
    ...
    if (newWidth > 20 && newWidth < 80) {
      ...
      document.querySelector(".preview-panel").style.flex =
        `0 0 ${100 - newWidth}%`;
    }
  });
